...
public class BluetoothLeService extends Service {
    ...
    private byte pkg[] = new byte[12 * 500];
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                Log.i(TAG, "Connected to GATT server, and start to discover Services.");
                // Attempts to discover services after successful connection.
                mBluetoothGatt.discoverServices();
                //mBluetoothGatt.requestMtu(36);
    ...
    private void broadcastUpdate(final String action,
        ...
        if (Const.UUID_CHARACTER_RECEIVE.equals(characteristic.getUuid())) {
            ...
            if (runFlag) {
                for (byte b : data) {
                    pkg[bufIndex++] = b;
                    if (bufIndex == pkg.length - 1) {
                        String str = "";
                        for (int i = 0; i < pkg.length; i++) {
                            if (pkg[i] == 0x55 && pkg[i + 1] == -86) {    // BERRY Protocol
                                //if (((int)pkg[i] & 0x80) > 0) {   // BCI Protocol
                                Log.i(TAG, str);
                                str = "";
                            }
                            //str += String.format("0x%02x ", pkg[i]);
                            str += String.format("%d ", pkg[i]);
                        }
                        isFinished = true;
                        break;
                    }
                ...
                for (byte b : data) {
                    ...
                    if (bufIndex == buf.length) {
                        ...
                        mRecvListener.onDataReceived(buf);
                        bufIndex = 0;
                    }
                }
            }
            ...
            Log.i(TAG, "broadcastUpdate: should never goes here!");
            final byte[] data = characteristic.getValue();
